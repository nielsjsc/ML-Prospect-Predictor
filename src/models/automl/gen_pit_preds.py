...
from src.data.matchProspect import process_prospect_data

def load_2024_data():
    """Load and process 2024 prospect data"""
    file_path = os.path.join(root_dir, 'src/data/raw/pitchers/2024.csv')
    df = pd.read_csv(file_path)
    
    print("\nLoaded columns:")
    print(df.columns.tolist())
    
    # Initial column renames
    ...
    df['Year'] = 2024
    df = process_prospect_data(df, 'pitcher')
    
    features_df = df[PITCHER_FEATURES + ['Name']].copy()
...
def predict_performance(model_id):
    ...
    try:
        ...
        sample_instance = predict_data[PITCHER_FEATURES].iloc[0].to_dict()
        print(sample_instance)
        print("\nData types:")
        print(predict_data[PITCHER_FEATURES].dtypes)
        ...
        instances = predict_data[PITCHER_FEATURES].to_dict('records')
...
if __name__ == "__main__":
    MODEL_ID = "projects/189414122511/locations/us-west1/models/4283525778001362944"  # New pitcher model ID
    predictions = predict_performance(MODEL_ID)
    
    output_path = os.path.join(root_dir, 'src/data/predictions/2024_pitcher_war_predictions.csv')
